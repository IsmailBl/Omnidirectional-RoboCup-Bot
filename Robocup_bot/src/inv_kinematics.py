...
def main():
	...
	while not rospy.is_shutdown():
		...
		omega2.data = (-np.sin(yaw+0.17777777777*math.pi)*x_dot + np.cos(yaw+0.17777777777*math.pi)*y_dot + base_diag*theta_dot)/wheel_radius
		omega3.data = (-np.sin(yaw+1.25*math.pi)*x_dot + np.cos(yaw+1.25*math.pi)*y_dot + base_diag*theta_dot)/wheel_radius
		omega4.data = (-np.sin(yaw+0.82222222222*math.pi)*x_dot + np.cos(yaw+0.82222222222*math.pi)*y_dot + base_diag*theta_dot)/wheel_radius
